@extends('layouts.app')
@section('title', 'Data Mahasiswa | SIPKM UM')
@section('style')
...
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <div class="post d-flex flex-column-fluid" id="kt_post">
        ...
        <div id="kt_content_container" class="container-fluid">
            ...
            <div class="card">
                <!--begin::Card header-->
                <x-organisms.card-header filter="true" add="true" addHref="{{ route('admin.student.edit') }}" />
                <!--end::Card header-->
                <!--begin::Card body-->
                <div class="card-body pt-0">
                    <!--begin::Table-->
                    <div id="studentsTable">
                        @include('pages.admin.student.table')
                    </div>
                    <!--end::Table-->
                </div>
                <!--end::Card body-->
        ...
        <div class="card-body" id="kt_explore_body">
            ...
            <div id="kt_explore_scroll" class="scroll-y me-n5 pe-5">
                <div class="d-grid gap-2 mb-2">
                    <span class="text-cap fw-bold fs-6">Angkatan</span>
                    <div class="d-grid gap-2" id="filter_angkatan_container">
                        <!-- Select Option dengan opsi "Semua Fakultas" -->
                        <select name="filter_angkatan" id="filter_angkatan" class="form-select" data-control="select2">
                            <option value="all">Semua Angkatan</option> <!-- Opsi "Semua Fakultas" -->
                            @foreach($angkatan as $index => $row)
                                <option value="{{ $row }}">20{{ $row }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="d-grid gap-2 mb-2">
                    ...
                    <div class="d-grid gap-2" id="filter_role_container">
                        ...
                        <div class="form-check">
                            <input type="radio" name="filter_role" id="filter_role_pkmcenter" value="{{ \App\Enums\RoleEnum::PKM_CENTER }}" class="form-check-input form-check-input-sm">
                            <label class="form-check-label" for="filter_role_pkmcenter">PKM Center</label>
                        </div>
                        <div class="form-check">
                            <input type="radio" name="filter_role" id="filter_role_student" value="{{ \App\Enums\RoleEnum::MAHASISWA }}" class="form-check-input form-check-input-sm">
                            <label class="form-check-label" for="filter_role_student">Mahasiswa</label>
                        </div>
                        <div class="form-check">
        ...
        $(document).on("click", ".btn-delete", function(){
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $("#frmDelete").attr("action", "{{ route('admin.student.single_destroy', '_id_') }}".replace("_id_", id));
...
<script>
    $(document).ready(function() {
        ...
        // Atur dropdown angkatan
        const selectedAngkatan = urlParams.get('filter_angkatan');
        if (selectedAngkatan) {
            $('#filter_angkatan').val(selectedAngkatan).trigger('change');
        }
        ...
        $('#searchInput').on('keyup', function() {
            ...
            const angkatanId = $('#filter_angkatan').val();
            if (angkatanId && angkatanId !== 'all') {
                params.append('filter_angkatan', angkatanId);
            }
        ...
        $(document).on('click', '.pagination a', function(event) {
            ...
            const angkatanId = $('#filter_angkatan').val();
            if (angkatanId && angkatanId !== 'all') {
                params.set('filter_angkatan', angkatanId);
            }
        ...
        $('#kt_explore_submit').on('click', function() {
            ...
            // Redirect ke route admin.student.index dengan parameter filter
            const url = "{{ route('admin.student.index') }}";
            const params = new URLSearchParams({
                filter_angkatan: angkatanId,
                filter_role: role,
        ...
        $('#kt_explore_reset').on('click', function() {
            // Redirect ke route admin.student.index tanpa parameter filter
            const url = "{{ route('admin.student.index') }}";
